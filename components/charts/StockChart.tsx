...
const RANGE_LABELS: Record<Range, string> = { '1d': '1D', '5d': '5D', '1mo': '1M', '3mo': '3M', '6mo': '6M', '1y': '1Y' }
...
export function StockChart({ ticker, candles: initialCandles }: Props) {
  ...
  useEffect(() => {
    ...
    const chart = createChart(chartRef.current, {
      layout: { background: { type: ColorType.Solid, color: '#111118' }, textColor: '#6b6b80' },
    ...
    if (chartType === 'candle') {
      mainSeries = chart.addSeries(CandlestickSeries, {
        upColor: '#00c87a', downColor: '#f5424b',
        borderUpColor: '#00c87a', borderDownColor: '#f5424b',
        wickUpColor: '#00c87a', wickDownColor: '#f5424b',
      })
      mainSeries.setData(
        candles.map((c) => ({ time: c.time as number, open: c.open, high: c.high, low: c.low, close: c.close }))
      )
    } else {
      mainSeries = chart.addSeries(LineSeries, { color: '#4b9eff', lineWidth: 2 })
      mainSeries.setData(candles.map((c) => ({ time: c.time as number, value: c.close })))
    }
    ...
    const observer = new ResizeObserver(() => chart.applyOptions({ width: chartRef.current?.clientWidth ?? 600 }))
  ...
  async function changeRange(r: Range) {
    ...
    const res = await fetch(`/api/chart/${encodeURIComponent(ticker)}?range=${r}&interval=${interval}`)
  ...
  return (
    <div className="rounded-lg overflow-hidden" style={{ border: '1px solid var(--border)' }}>
      ...
      {loading && <div className="flex items-center justify-center h-[380px]" style={{ color: 'var(--text-muted)' }}>Loading...</div>}
      <div ref={chartRef} className={loading ? 'hidden' : 'chart-container'} />
    </div>
  )
}
